...
class PluginsidController extends Base
{
    ...
    public function saveDataAction(){
        ...
        $data['plugin_id'] = input('post.plugin_id',0,'int');

        !$data['plugin_id'] && jsonError('无效的参数');

        $data['sid'] = input('post.sid',0,'int');

        $data['class_id'] = input('post.class_id',0,'int');
        $data['subcategory_id'] = input('post.subcategory_id',0,'int');
        $data['category_id'] = input('post.category_id',0,'int');
        $data['reliability'] = input('post.reliability',1,'int');
        $data['priority'] = input('post.priority',1,'int');

        $data['name'] = input('name');
        $old_id  = input('old_sid',0,'int');
        !$data['name'] && jsonError('无效的参数');

        $edit = input('post.edit',0,'int');
