...
class WisdomenergyController extends Base
{
    ...
    public function saveConfigAction()
    {
        ...
        if (!\Tools::isEmpty($param['menus'])) {
            ...
            //$param['detail'] = $param['menus'];
            //$param['detail'] = $this->wisdomenergy->UpdateMenu($param);
...
//        $param['detail'] = decryptcode(input('post.detail'));
//        $param['detail'] = input('post.detail');
//        $param['id'] = input('post.id');
//        if (!\Tools::isEmpty($param['detail'])) {
//            $param['detail'] = html_entity_decode($param['detail']);
//
//            $param['detail'] = $this->appendTs($param['detail']);
//            $res = $this->wisdomenergy->SaveMenuData($param);
//
//            if ($res > 0) {
//                $this->logger(98);
//                jsonResult();
...
//        $params['aggregations']['tag']['aggregations']['top']['top_hits']['sort']['ts']['order'] = 'desc';
       ...
       // echo json_encode($params,256);die;
        ...
        $collect = $this->es->query('zhny-smart-collect', $params);  //采集
...
//删除模板
    public function DelMenuAction()
    {
        if (!\Tools::isEmpty(input('get.id'))) {
            $res = $this->wisdomenergy->DelMenu(input('get.id'));
            if ($res > 0) {
                $this->logger(100);
                jsonResult();
            }
    ...
    public function testAction(){
        ...
        //$params = '{"from": 0,"size": 30,"sort": [{"ts": {"order": "desc"}}]}';
        $params['aggregations']['tag']['terms']['field'] = 'tag';
       $params['aggregations']['tag']['aggregations']['top']['top_hits']['size'] = 1;
       $params['aggregations']['tag']['aggregations']['top']['top_hits']['sort']['ts']['order'] = 'desc';
        /*
        * @zhny_enterprise_report
        * @zhny-smart-collect


        ### curl

        http://19.19.19.11/api.php/wisdomenergy/test?tag=WRWJXYC&ts=2020-07-03 10:41:23|2020-07-05 10:41:23
        
        */
         //$ret = $ES->getList('zhny_enterprise_report',$params);
         $ret = $ES->query('zhny-smart-collect',$params);
       ...
       /**
       /wisdomenergy
       * 增加数据   /test?tag=WRWJXYC&ts=2020-07-03 10:41:23|2020-07-05 10:41:23
       */
       $params1 = [
            'vendor' => 'MAGUS11',
            'dev' => rand(1,10),
       ...
       $delete = [
        "_id" => "jF3IKHMBiEm-ua1b_u3xz"
       ];
       //var_dump($ES->add('zhny_enterprise_report',$params1));
       //var_dump($ES->replace('zhny_enterprise_report','jF3IKHMBiEm-ua1b_u3xz',$params1));
       var_dump($ES->delete('zhny_enterprise_report',$delete));
