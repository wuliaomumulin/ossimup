...
class UserController extends Base
{
    ...
    */
    ...
    public function authopenAction(){
        $id = decryptcode(input('post.id'));
        $status = decryptcode(input('post.status'));
        $user = new User();
        if($status != 1){
            $user->where(['id' => $id])->setField('status',1);
            $this->logger(42);
            jsonResult('权限开通');
        }else{
            $user->where(['id' => $id])->setField('status',0);
            $this->logger(43);
            jsonResult('权限关闭');
        }
    ...
    */
    public function setUserInfoAction(){
        ...
        if($this->getRequest()->isGet()){
            ...
            $uid = $_SESSION['uid'];
            $rs = $this->model->field('nickname,email,mobile,usb_key')->where(['id'=>$uid])->find();
