...
class LoginController extends \Yaf\Controller_Abstract
{
    ...
    public function checkloginAction()
    {
        ...
        $configsystem = $Configsystem->getTimeOutFailCount();
      ...
      //  echo $login_num;die;
        try {
            $result = $model->checkLogin($username, $userpwd, $vcode, $login_num,$configsystem);
            ...
            if ($e->getCode() == 4 || $e->getCode() == 3) {
                ...
                if ($login_num < $configsystem['fail_count']) {
                    ...
                    // 等于5的话   保留20MIN
                } elseif ($login_num ==  $configsystem['fail_count']) {
                    self::loggerr(94,'',$username);
                    $time = $configsystem['fail_time'] * 60;
                    $redis->set($key, $configsystem['fail_count'], 0, 0, $time);
                }
    ...
    protected function loggerr($code, $info = '',$username)
    {
        $logMsg = new LogMsg();
        $status = $logMsg->getStatus($code);
        if ($status['status'] == 1) {
            $msg = $logMsg->getSysLogMsg($code);
            $log = new Log();
            $da = array('user_name' => $username, 'log_event' => $info == '' ? $msg['name'] : $info, 'log_ip' => get_client_ip(), 'remark' => $msg['msg']);
            return $log->syslogadd($da);
        }
