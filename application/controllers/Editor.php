...
class EditorController extends Base{
    ...
    protected $editor;
    protected $plugins_path = "/work/etc/agent/plugins/";
    protected $num = 30;
    ...
    public function getEnabledPluginAction(){
        ...
        //xss过滤事件
        $name = $this->editor->xss_filter($name);
        ...
        foreach ($content as $k => $v) {
            ...
            $explode =  explode('=', $v);
            $rs[$k]['file_path'] = $explode[count($explode)-1];
            $rs[$k]['path'] = $rs[$k]['realname'] = $rs[$k]['name'];
        }
    ...
    public function setEnabledPluginAction(){
        ...
        foreach ($enabled as $k => &$v){
            $v = substr($v,strpos($v,"=")+1)."=".$this->plugins_path.substr($v,strpos($v,"=")+1).".cfg";
        }
