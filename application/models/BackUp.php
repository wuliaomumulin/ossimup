...
class BackUp extends Model
{
    ...
    public function getNowHaveLog()
    {
        ...
        $redis = new \phpredis();
        if($redis->get('getNowHaveLog')){
            $rs = json_decode($redis->get('getNowHaveLog'),1);
        }else{
            $query = "SELECT DATE(timestamp) as day FROM alienvault_siem.acid_event GROUP BY day ORDER BY day DESC";
            $rs = $this->query($query);
            $redis->set('getNowHaveLog',json_encode($rs,256),0,0,400);
        }
...
//        }
    ...
    public function getNowHaveAudit()
    {
        ...
        $redis = new \phpredis();
        if($redis->get('getNowHaveAudit')){
            $rs = json_decode($redis->get('getNowHaveAudit'),1);
        }else{
            $query = "SELECT DATE(insert_time) as day FROM alienvault.sys_log GROUP BY day ORDER BY day DESC";
            $rs = $this->query($query);
            $redis->set('getNowHaveAudit',json_encode($rs,256),0,0,400);
        }
        return $rs;
    }
