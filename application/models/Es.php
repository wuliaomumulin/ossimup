...
class Es extends Model{
     ...
     * @return [type]         [返回一个数组]
     */
    public function getList($tables = 'zhny_smart_collect',$params = []){
     ...
     * 独立查询
     */
    public function query($tables = 'zhny_smart_collect',$params = []){
     ...
     * 如果数组的key不存在是会自动创建的
     */
    public function add($tables = 'zhny_smart_collect',$params = []){

        if (!is_array($params) || empty($params)) return '无效的参数';

        $spHttp = new \spHttp();
        //$config = \Yaf\Registry::get("config");
        $url = $_SESSION['ES_HOSTNAME']. '/'.$tables.'-'.date('Ymd').'/_doc';
        $head="Content-Type: application/json";
     ...
     * 当传递的id存在为替换，不存在则创建
     */
    public function replace($tables = 'zhny_smart_collect',$id='',$params = []){
        ...
        $spHttp = new \spHttp();
        //$config = \Yaf\Registry::get("config");
        $url = $_SESSION['ES_HOSTNAME']. '/'.$tables.'-'.date('Ymd').'/_doc/'.$id;
        $head="Content-Type: application/json";
     ...
     */
    public function delete($tables = 'zhny_smart_collect',$input = []){
     ...
     * 独立查询
     */
    public function count($tables = '',$params = []){
        ...
        $url = $_SESSION['ES_HOSTNAME']. '/'.$tables.'*/_count';
        $head="Content-Type: application/json";
        $ret = json_decode($spHttp->vpost($url, $params, $head),256);
        return is_null($ret['count']) ? 0 : $ret['count'];
    }
     ...
     * @return [type]         [返回一个数组]
     */
    public function getAggregationsList($tables = 'zhny_smart_collect',$params = [],$aggregations = ''){
     ...
     * @return [type]         [返回一个数组]
     */
    public function getOriginalList($tables = 'zhny_smart_collect',$params = []){
