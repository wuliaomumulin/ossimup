...
class Plugin extends Model
{
    ...
    public function getAll($where = [], $order = 'a.id desc'): ?array
    {
        //field
    ...
    {
        if (decryptcode($param['edit']) == 0) {
            unset($param['edit']);
            $data['name'] = decryptcode($param['name']);
            $data['id'] = decryptcode($param['id']);
            $data['product_type'] = decryptcode($param['product_type']);
            $data['vendor'] = decryptcode($param['vendor']);
            $data['description'] = decryptcode($param['description']);
            $res = $this->add($data);

        } elseif (decryptcode($param['edit']) == 1) {
            unset($param['edit']);
            $id = decryptcode($param['ID']);
            unset($param['ID']);
            unset($param['type']);
