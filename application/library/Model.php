...
class Model {
    ...
    public function getComment($field = ''){
        $schema = $this->dbName;
        $table = $this->trueTableName;
        $sql = "select column_name as `key`,column_comment as `description` from information_schema.columns where 1";
        if(!empty($schema)) $sql.= " and table_schema = '{$schema}'";
        if(!empty($table)) $sql.= " and table_name = '{$table}'";
        if(!empty($field)) $sql .= " and column_name = '{$field}'";

        return $this->db->query($sql);
    } 
    /**
    * @params $key String 键名
    * @params $data Array 键值，一个列表数组
    * @params $expire Int 失效时间
    * 设置缓存
    */
    public function setCache($key,$data,$expire=60){
        $redis = new \phpredis();
        
        $redis->set($key,json_encode($data),0,0,$expire); 
    } 
}
