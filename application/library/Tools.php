...
class Tools {
	...
	public static function filter($string, $force = 1, $allow='') {
        if($force) {
            if(is_array($string)) {
                ...
                //过滤sql注入
                $sqlstr = array("chr(","'","%27","union"," and ","%20and%20","+and+","version()","select","insert","delete","update","create");
                for ($i=0; $i < count($sqlstr); $i++) { 
                 if (strpos($string,$sqlstr[$i])!==false){
                   Fun::msg("请勿提交非法数据！");
                   exit();
                 }
                }
	 ...
	 * 首尾截取字符串
	 */
	public static function strCut($string,$strstart,$strend)
    {
	...
	public static function isEmpty($field) {
		return (!isset($field) || is_null($field) || empty($field) || $field === '' || $field === null);
	}
    ...
    * return Bool 是或者否
    */
    public static function is_single_array($array){

        return (count($array) == count($array,1)) ? Ture : False;
