 ...
 */
 class spUploadFile {
 	...
 	public $session_upload_progress = '';//上传文件进度条 
 	...
 	public function __construct($save_path = './upload/',$file_name = 'date',$allow_types = '') {
 		// $save_path = constant('APP_PATH') . '/public/uploads/'.$_SESSION['web_id'].'/'.$save_path.'/';//上传文件保存路径
		...
		$this->max_size = substr(ini_get('upload_max_filesize'),0,-1);
 		$this->session_upload_progress = ini_get('session.upload_progress.prefix').ini_get('session.upload_progress.name');
 	}
 	 ...
 	 */
	public function upload_file($files,$types = '',$uploadpath = '',$savepath = '') {
		if (!empty($savepath)){
		...
		switch ($error) {
			case 0 : $this->errmsg = '';
				break;
			case 1 : $this->errmsg = 'Error:超过了php.ini中文件大小';
				break;
			case 2 : $this->errmsg = 'Error:超过了MAX_FILE_SIZE 选项指定的文件大小';
				break;
	 	    case 3 : $this->errmsg = 'Error:文件只有部分被上传';
				break;
			case 4 : $this->errmsg = 'Error:没有文件被上传';
				break;
			case 5 : $this->errmsg = 'Error:上传文件大小为0';
				break;
		    default : $this->errmsg = 'Error:上传文件失败！';
				break;
			}
		if($error == 0 && is_uploaded_file($tmp_name)) {
			//检测文件类型
			if($this->check_file_type($name,$types) == FALSE){
				$this->errmsg = 'Error:不支持的文件类型';
				return $this->errmsg;
			}
			//复测文件类型
			if($files['type'] != 'application/x-zip-compressed'){
				$this->errmsg = 'Error:不支持的文件类型';
				return $this->errmsg;
			}

			//检测文件大小
			$max_size = ceil($this->max_size*1024*1024);
			if($size > $max_size){
				$this->errmsg = 'Error:上传文件太大，最大支持'.$max_size.'kb的文件';
				return $this->errmsg;
			}
			...
			if(move_uploaded_file($tmp_name,$this->uploaded)){
                ...
                //return str_replace(constant('APP_PATH') ."/public", "", $this->uploaded);
				return str_replace("../html", "", $this->uploaded);
			}else{
				$this->errmsg = 'Error:文件上传失败！';
				return $this->errmsg;
			}
	...
	/**
	* 获得上传进度
	* @params $return upload_progress 为数字
	*/
	public function upload_progress(){
		$session_upload_progress = $_SESSION[$this->session_upload_progress];
		if(empty($session_upload_progress)){
			$this->errmsg = '文件已上传完成';
			return 100;
		}
		$this->errmsg = '文件正在上传中';
		return ceil($session_upload_progress['bytes_processed'] / $session_upload_progress['content_length'] * 100); 
	...
	/**
	* @access public base64转图片
	* params string $file 保存路径	
	* params string $str base64图片
	*/
	public function base64toimg($file,$str){
		if(preg_match('/^(data:\s*image\/(\w+);base64,(.*))/',$str,$result)){
		  	 //装饰图片后缀
			  $ext = $result[2]=='jpeg' ? 'jpg' : $result[2];
			  $file .= '.'.$ext;
			  file_put_contents($file,base64_decode($result[3]));
	          $this->errmsg = $file;
	          return $this->errmsg;
		}else{
		  $this->errmsg = 'base64图片格式不合法';
		  return FALSE;
		}
	}
