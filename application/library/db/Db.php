...
class db_Db {
    ...
    public function factory($db_config='') {        
        // 读取数据库配置
        $this->config = $db_config = $this->parseConfig($db_config);
        ...
        if ( !empty($db_config) && is_string($db_config)) {
            ...
            $db_config = $this->parseDSN($db_config);
             ...
             $db_config = array(
                  ...
                  'deploy'    =>  isset($db_config['deploy_type'])?$db_config['deploy_type']:0,
                  'rw_separate'=> isset($db_config['rw_separate'])?$db_config['rw_separate']:false,
                  'master_num'=>  isset($db_config['master_num'])?$db_config['master_num']:1,
                  'slave_no'  =>  isset($db_config['slave_no'])?$db_config['slave_no']:'',
                  'bind_param'=>  isset($db_config['bind_param'])?$db_config['bind_param']:false,
                  'like_fields'=> isset($db_config['like_fields'])?$db_config['like_fields']:'',
                  'debug'     =>  isset($db_config['debug'])?$db_config['debug']:@APP_DEBUG,
                  'lite'      =>  false,
             );
    ...
    protected function parseSet($data) {
        foreach ($data as $key=>$val){
            if(is_array($val) && 'exp' == $val[0]){
              ...
              if($this->config['bind_param'] && 0 !== strpos($val,':')){
                $name   =   md5($key);
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
            ...
            //对字符串类型字段采用模糊匹配
            if($this->config['like_fields'] && preg_match('/^('.$this->config['like_fields'].')$/i',$key)) {
                $val  =  '%'.$val.'%';
